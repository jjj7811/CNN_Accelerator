...
typedef struct {
    ...
    XMy_conv_uio_map maps[ MAX_UIO_MAPS ];
} XMy_conv_uio_info;

/***************** Variable Definitions **************************************/
static XMy_conv_uio_info uio_info;
...
static int uio_info_read_name(XMy_conv_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMy_conv_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMy_conv_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMy_conv_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMy_conv_Initialize(XMy_conv *InstancePtr, const char* InstanceName) {
	XMy_conv_uio_info *InfoPtr = &uio_info;
